<?php

namespace App\Actions;

use App\Models\User;
use App\Repositories\Interfaces\ListingRepositoryInterface;

class CheckTariffLimitAction
{
    public function __construct(
        private readonly ListingRepositoryInterface $listingRepository,
    ) {}

    /**
     * Квота тарифа на объявления: занято = pending + approved
     * (rejected и удалённые место не занимают).
     *
     * @throws \Symfony\Component\HttpKernel\Exception\HttpException 403 при исчерпании
     */
    public function execute(User $user): void
    {
        $tariff = $user->activeTariff();

        $used = $this->listingRepository->countByUserAndStatuses($user->id, ['pending', 'approved']);

        if (! $tariff || $used >= $tariff->listings_limit) {
            abort(403, __('messages.tariff_limit_exceeded'));
        }
    }
}
