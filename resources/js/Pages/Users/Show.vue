  ...
  <AppLayout>
    <template #header>
      <div class="flex items-center gap-2">
        <Link :href="route('users.index')" class="text-muted hover:text-blue transition text-[14px]">Пользователи</Link>
    ...
    <div class="grid gap-5" style="grid-template-columns: 320px 1fr;">
      ...
      <div class="space-y-4">
        <div class="rounded-card bg-white shadow-soft dark:bg-dcard p-6">
          ...
          <div class="space-y-2.5 text-[13px]">
            <div class="flex justify-between"><span class="text-muted font-semibold">Пол</span><span class="font-bold text-ink dark:text-slate-200">{{ user.gender === 'male' ? 'Мужской' : user.gender === 'female' ? 'Женский' : '—' }}</span></div>
            <div class="flex justify-between"><span class="text-muted font-semibold">Дата рождения</span><span class="font-bold text-ink dark:text-slate-200 font-data">{{ user.birth_date ? formatDate(user.birth_date) : '—' }}</span></div>
            <div class="flex justify-between"><span class="text-muted font-semibold">Регион</span><span class="font-bold text-ink dark:text-slate-200">{{ user.region?.name_ru || '—' }}</span></div>
            <div class="flex justify-between"><span class="text-muted font-semibold">Город</span><span class="font-bold text-ink dark:text-slate-200">{{ user.city?.name_ru || '—' }}</span></div>
            <div class="flex justify-between"><span class="text-muted font-semibold">Тариф</span><span class="font-bold text-ink dark:text-slate-200">{{ user.tariff?.name || 'Бесплатный' }}</span></div>
            <div class="flex justify-between"><span class="text-muted font-semibold">Дата рег.</span><span class="font-bold font-data text-ink dark:text-slate-200">{{ formatDate(user.created_at) }}</span></div>
          </div>

          <div v-if="user.blocked_reason" class="mt-4 rounded-btn bg-red/10 p-3 text-[12px] font-semibold text-red">
            <strong>Причина блокировки:</strong> {{ user.blocked_reason }}
          </div>

          <div v-if="user.note" class="mt-4 rounded-btn bg-surface p-3 text-[12px] text-muted dark:bg-dbg">
            <strong>Примечание:</strong> {{ user.note }}
          </div>
        </div>

        <!-- Stats -->
        <div class="rounded-card bg-white shadow-soft dark:bg-dcard p-5">
          <h3 class="text-[13px] font-extrabold text-ink dark:text-slate-100 mb-3 uppercase tracking-wide">Статистика</h3>
          <div class="grid grid-cols-3 gap-3 text-center">
            <div>
              <div class="font-data text-[20px] font-black text-blue">{{ stats.listings }}</div>
              <div class="text-[11px] text-muted font-semibold">Объявл.</div>
            </div>
            <div>
              <div class="font-data text-[20px] font-black text-teal">{{ stats.videos }}</div>
              <div class="text-[11px] text-muted font-semibold">Роликов</div>
            </div>
            <div>
              <div class="font-data text-[20px] font-black text-red">{{ stats.complaints }}</div>
              <div class="text-[11px] text-muted font-semibold">Жалоб</div>
            </div>
          </div>
        </div>
      ...
      <div class="rounded-card bg-white shadow-soft dark:bg-dcard overflow-hidden">
        <div class="px-[22px] py-[18px] border-b border-line dark:border-dline">
          <span class="text-[15px] font-extrabold text-ink dark:text-slate-100">Объявления пользователя</span>
        </div>
        <table class="w-full">
          ...
          <tbody>
            ...
            <tr v-if="!userListings?.length">
              <td colspan="5" class="px-4 py-8 text-center text-[13px] text-muted">Объявлений нет</td>
            </tr>
          </tbody>
        </table>
      </div>
