  ...
  <AppLayout>
    <template #header>
      <div class="flex items-center gap-2 text-[14px]">
        <Link :href="route('listings.index')" class="text-muted hover:text-blue transition">Объявления</Link>
    ...
    <div class="grid gap-5" style="grid-template-columns: 1fr 360px;">
      ...
      <div class="space-y-5">
        ...
        <div class="rounded-card bg-white shadow-soft dark:bg-dcard p-5">
          <h3 class="text-[15px] font-extrabold text-ink dark:text-slate-100 mb-4">Медиафайлы</h3>
          <div v-if="listing.media?.length">
            <div class="rounded-[12px] overflow-hidden bg-surface dark:bg-dbg aspect-video mb-3">
              <img :src="`/storage/${listing.media[activePhoto]?.path}`" class="w-full h-full object-cover" :alt="listing.title" />
            </div>
        ...
        <!-- Details -->
        <div class="rounded-card bg-white shadow-soft dark:bg-dcard p-5">
          <h3 class="text-[15px] font-extrabold text-ink dark:text-slate-100 mb-4">Информация</h3>
          <div class="grid grid-cols-2 gap-4 text-[13px]">
            <div><span class="text-muted font-semibold">Название:</span><br><span class="font-bold text-ink dark:text-slate-200">{{ listing.title }}</span></div>
            <div><span class="text-muted font-semibold">Тип:</span><br><span class="font-bold text-ink dark:text-slate-200">{{ listing.type === 'goods' ? 'Товар' : 'Услуга' }}</span></div>
            <div><span class="text-muted font-semibold">Цена:</span><br><span class="font-data font-bold text-ink dark:text-slate-200">{{ listing.price ? Number(listing.price).toLocaleString('ru') + ' ₺' : 'Договорная' }}</span></div>
            <div><span class="text-muted font-semibold">Категория:</span><br><span class="font-bold text-ink dark:text-slate-200">{{ listing.category?.name_ru || '—' }}</span></div>
            <div><span class="text-muted font-semibold">Регион:</span><br><span class="font-bold text-ink dark:text-slate-200">{{ listing.region?.name_ru || '—' }}, {{ listing.city?.name_ru || '—' }}</span></div>
            <div><span class="text-muted font-semibold">Телефон:</span><br><span class="font-data font-bold text-ink dark:text-slate-200">{{ listing.phone }}</span></div>
            <div><span class="text-muted font-semibold">Просмотры:</span><br><span class="font-data font-bold text-ink dark:text-slate-200">{{ listing.views || 0 }}</span></div>
            <div><span class="text-muted font-semibold">Дата:</span><br><span class="font-data font-bold text-ink dark:text-slate-200">{{ formatDate(listing.created_at) }}</span></div>
          </div>
          <div v-if="listing.description" class="mt-4 pt-4 border-t border-line dark:border-dline">
            <p class="text-[12px] font-semibold text-muted mb-1">Описание:</p>
      ...
      <div class="space-y-4">
        <!-- Author -->
        <div class="rounded-card bg-white shadow-soft dark:bg-dcard p-5">
          <h3 class="text-[13px] font-extrabold text-ink dark:text-slate-100 mb-3 uppercase tracking-wide">Автор</h3>
          ...
          <div class="mt-3">
            <Link :href="route('users.show', listing.user_id)" class="text-[12px] font-bold text-blue hover:underline">Профиль пользователя →</Link>
          </div>
        </div>

        <!-- Moderation -->
        <div class="rounded-card bg-white shadow-soft dark:bg-dcard p-5">
          <h3 class="text-[13px] font-extrabold text-ink dark:text-slate-100 mb-3 uppercase tracking-wide">Модерация</h3>
    ...
    <div v-if="showRejectModal" class="fixed inset-0 z-[600] flex items-center justify-center bg-black/40 backdrop-blur-sm" @click.self="showRejectModal = false">
      <div class="w-[440px] rounded-card bg-white p-6 shadow-[0_24px_48px_rgba(0,0,0,.18)] dark:bg-dcard">
        ...
        <div class="flex gap-2.5">
          <button @click="showRejectModal = false" class="flex-1 rounded-btn border-2 border-line py-[11px] text-[13px] font-bold text-muted hover:border-blue hover:text-blue transition dark:border-dline">Отмена</button>
